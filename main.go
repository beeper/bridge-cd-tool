...
func githubMain() {
	if branch := env("GITHUB_REF_NAME"); branch != "main" && branch != "master" && branch != "xchat" && branch != "megadiscord" {
...
func gitlabMain() {
	...
	isLatest := branch == "main" || branch == "master" || branch == "xchat" || branch == "megadiscord"
	bridgeType := BridgeType(env("BEEPER_BRIDGE_TYPE"))
